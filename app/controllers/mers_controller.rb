class MersController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @mer.save
        ...
        format.json { render :show, status: :created, location: @mer }
      else
  ...
  def update
    respond_to do |format|
      if @mer.update(mer_params)
        ...
        format.json { render :show, status: :ok, location: @mer }
      else
  ...
  private
    ...
    def mer_params
      params.require(:mer).permit(:polling_station_id, :num_mer, :name)
    end
end
